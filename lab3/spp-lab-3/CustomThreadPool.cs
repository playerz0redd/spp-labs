    ...
    public class MyThreadPool : IDisposable
    {
        ...
        private void WorkerLoop(int workerId)
        {
            ...
            while (true)
            {
                ...
                lock (_lockObj)
                {
                    while (_taskQueue.Count == 0 && !me.ShouldExit && !_isDisposed)
                    {
                        Monitor.Wait(_lockObj, 500);
                    }
                ...
                {
                    LogPoolEvent($"err in {me.Id}: {ex.Message}", ConsoleColor.Red);
                }
                finally
                {
                    lock (_lockObj)
                    {
                        me.IsWorking = false;
                        me.LastFinishedWork = DateTime.Now;
                    }
                }
            }
        }
        ...
        private void SupervisorLoop()
        {
            while (!_isDisposed)
            {
                ...
                lock (_lockObj)
                {
                    ...
                    int activeWorkers = _workers.Count(w => w.IsWorking);
                    int totalWorkers = _workers.Count;

                    LogPoolEvent($"[stat] threads: {totalWorkers} (work: {activeWorkers}), q: {queueCount}", ConsoleColor.Cyan);

                    if (queueCount > 0 && activeWorkers == totalWorkers && totalWorkers < _maxThreads)
                    {
                        LogPoolEvent($"[+] q {queueCount}, adding thread", ConsoleColor.Yellow);
                        CreateWorker();
                    }

                    if (totalWorkers > _minThreads)
                    {
                        var idleWorkers = _workers.Where(w => !w.IsWorking && (DateTime.Now - w.LastFinishedWork) > _idleTimeout).ToList();
                        foreach (var worker in idleWorkers)
                        {
                            if (_workers.Count <= _minThreads) break;
                            LogPoolEvent($"[-] worker {worker.Id} idle, removing", ConsoleColor.DarkYellow);
                            worker.ShouldExit = true;
                        }
                    }

                    var hungWorkers = _workers.Where(w => w.IsWorking && (DateTime.Now - w.CurrentWorkStartTime) > _hungTimeout).ToList();
                    foreach (var zombie in hungWorkers)
                    {
                        LogPoolEvent($"[!] worker {zombie.Id} hung, replacing", ConsoleColor.Magenta);
        ...
        private void LogPoolEvent(string message, ConsoleColor color)
        {
            lock (Console.Out)
            {
        ...
        public void WaitAllAndDispose()
        {
            while (true)
            {
                lock (_lockObj)
                {
                    if (_taskQueue.Count == 0 && _workers.All(w => !w.IsWorking)) break;
                }
                Thread.Sleep(200);
            }
